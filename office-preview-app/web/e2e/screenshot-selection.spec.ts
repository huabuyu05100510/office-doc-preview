...
test('选中测试：拖动鼠标覆盖一段文字', async ({ page, request }) => {
  ...
  // 选中 "前端工程师" 一段
  const target = page.locator('.pdf-text-layer p').filter({ hasText: '前端工程师' }).first()
  ...
  await page.waitForTimeout(500)
  await page.screenshot({ path: 'test-results/alignment-selection.png', fullPage: false })
})
