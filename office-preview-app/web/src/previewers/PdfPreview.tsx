...
// 集成自研预测渲染引擎
import { createPredictiveEngine, PredictiveRenderEngine } from '../utils/predictive-render'
...
interface Props { url: string; docSize?: number; task?: Task }
...
export function PdfPreview({ url, docSize = 0, task: _task }: Props) {
  ...
  useEffect(() => {
    ...
    const loadingTask = pdfjsLib.getDocument({
      ...
      disableStream: false,
      // 修复字体映射错误：添加cMapUrl和cMapPacked配置
  ...
  // 集成预测渲染引擎
  const predictiveEngineRef = useRef<PredictiveRenderEngine | null>(null)

  useEffect(() => {
    // 初始化预测引擎
    predictiveEngineRef.current = createPredictiveEngine()
    return () => predictiveEngineRef.current?.reset()
  }, [])
  ...
  useEffect(() => {
    ...
    const onScroll = () => {
      ...
      // 使用预测渲染引擎记录滚动事件
      predictiveEngineRef.current?.recordScroll(el.scrollTop)

      // 获取预测统计信息
      const stats = predictiveEngineRef.current?.getStats()
      usePerf.getState().set({
        scrollVel: Math.round((Math.abs(el.scrollTop - last) / (now - lastT)) * 1000),
        predictiveLevel: stats?.level || 'idle',
        predictiveBuffer: stats?.buffer || 2
      })
