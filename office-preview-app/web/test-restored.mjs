...
if (firstSpan) {
  const cs = await firstSpan.evaluate(el => {
    ...
    return { x: Math.round(r.x), y: Math.round(r.y), w: Math.round(r.width), h: Math.round(r.height), opacity: cs.opacity, color: cs.color, fontSize: cs.fontSize, pointerEvents: cs.pointerEvents }
  })
...
await page.screenshot({ path: '/tmp/restored.png' })
await browser.close()
