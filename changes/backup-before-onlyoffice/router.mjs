...
// 创建任务并落盘原文件
export function createTaskFromFile({ id, name, size, mtime, ext, strategy, originalPath }) {
  const taskId = id || uid() // 支持传入ID，避免重复生成
  const now = Date.now()
  const task = {
    id: taskId,
    name,
...
export async function route(req, res) {
  ...
  if (m && req.method === 'GET') {
    ...
    if (!task) {
      console.log(`[file-service] Task not found: ${id}`)
      return sendJSON(res, 404, { error: 'task not found' })
    }
    const filePath = as === 'original' ? task.originalPath : task.previewPath || task.originalPath
    console.log(`[file-service] Serving: id=${id}, as=${as}, path=${filePath}`)
